...
If two threads are working on the same counter object, one is calling decrement and the other is calling increment, whichever thread is able to acquire the lock on this first will finish its work first and then let the other thread work on this.
